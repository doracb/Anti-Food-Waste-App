...
exports.createFood = async (req, res) => {
    try {
        const { name, category, quantity_value, quantity_unit, expiration_date, user_id } = req.body;
        if (!name || !category || !expiration_date || !user_id) {
            return res.status(400).json({ message: 'Missing required fields' });
        }
        if (quantity_value != null && quantity_value < 0.1) {
            return res.status(400).json({ message: 'Quantity value must be a number greater than 0.1' });
        }
